          ...
          <AnimatePresence mode="wait">
            {selectedTechnique ? (
              ...
              >
                ...
                <div>
                  <h4 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-1.5">
                    <ArrowRight className="w-3 h-3" /> Evidence & Context
                  </h4>
                  {selectedTechnique.matched_indicators?.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic mb-4">No specific evidence extracted.</p>
                  )}

                  {selectedTechnique.related_apis?.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-1.5">
                        <ArrowRight className="w-3 h-3" /> Related APIs
                      </h4>
                      <div className="flex flex-wrap gap-2">
                        {selectedTechnique.related_apis.map((api: string, i: number) => (
                          <span key={i} className="bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-mono px-2 py-1 rounded">
                            {api}
                          </span>
                        ))}
                      </div>
                  ...
                  {selectedTechnique.related_iocs?.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-1.5">
                        <ArrowRight className="w-3 h-3" /> Related IOCs
                      </h4>
                      <div className="flex flex-wrap gap-2">
                        {selectedTechnique.related_iocs.map((ioc: string, i: number) => (
                          <span key={i} className="bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs font-mono px-2 py-1 rounded">
                            {ioc}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div className="bg-muted/10 border border-border/50 rounded-lg p-3">
                        <span className="text-[10px] uppercase text-muted-foreground font-bold block mb-1">Confidence</span>
                        <span className="text-sm font-semibold">{selectedTechnique.confidence || 'Medium'}</span>
                    </div>
                    <div className="bg-muted/10 border border-border/50 rounded-lg p-3">
                        <span className="text-[10px] uppercase text-muted-foreground font-bold block mb-1">Source</span>
                        <span className="text-sm font-semibold">{selectedTechnique.source || 'AI Analyst'}</span>
                    </div>
                  </div>
                </div>
              </motion.div>
